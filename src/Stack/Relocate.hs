...
data RelocateOpts
    = RelocateOpts (Path Abs Dir) (Path Abs Dir) (Maybe (Path Abs Dir))
...
relocate (RelocateOpts dir dest maybeSrc) = do
    files <- findFiles dir (const True) (const True)

    let src = case maybeSrc of
                  Nothing -> dir
                  Just src' -> src'
    ...
    actions <- fmap catMaybes $ forM files $ \file -> do
        ...
        let otherFiles = return $ Just BinarySlashRep
    ...
    nrFiles <- fmap sum $ forM actions $ \(fp, rep) -> do
        ...
        case rep of
            ...
            (Just TextRep) -> do
                content <- B8.readFile fps
                let modified = DBS.replace srcPathAsBS destStrB content
                B8.writeFile fps $ B.concat $ BL.toChunks modified
                return 1
